...
def tune_maximum_market_share(df):
    ...
    """
    df = df.astype({col:'float64' for col in ['adopters_cum_last_year','developable_agent_weight','max_market_share','payback_period']})
...
def differential_Bass(x, p, q, mms):
    ...
    Returns
    -------
    Pandas series of floats: vector of market share at time t+2
    """

    return np.clip(2*(-q*x*x + (0.5+q-p)*x + p), 0, mms)
...
def market_grouper(county_attr, df, grouping_method, kmeans_vars=[], exclude_zeros=True, verbose=False):
    ...
    kmeans_vars : list
        column names in df or county_attr on which to cluster when using "kmeans" grouping_method
